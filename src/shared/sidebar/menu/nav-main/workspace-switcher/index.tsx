...
export function WorkspaceSwitcher({
  ...
  if (!Array.isArray(workspaces) || workspaces.length === 0) {
    console.error("[WorkspaceSwitcher] No workspaces provided or workspaces is not an array.");
    return (
      <SidebarMenu>
        <SidebarMenuItem>
          <div className="text-red-500 p-2">No workspaces available.</div>
        </SidebarMenuItem>
      </SidebarMenu>
    );
  }
  ...
  if (!activeWorkspace) {
    console.error("[WorkspaceSwitcher] No active workspace found.");
    return (
      <SidebarMenu>
        <SidebarMenuItem>
          <div className="text-red-500 p-2">No active workspace found.</div>
        </SidebarMenuItem>
      </SidebarMenu>
    );
  }
            ...
            <DropdownMenuTrigger asChild>
              ...
              >
                ...
                <div className="grid flex-1 text-left text-sm leading-tight">
                  ...
                  >
                    {typeof activeWorkspaceId === 'string' && activeWorkspaceId
  ? getWorkspaceType(activeWorkspaceId)
  : "No workspace selected"}
            ...
            >
              ...
              {workspaces.map((workspace, index) => {
                const WorkspaceIcon = getWorkspaceIcon(index, workspace.description ?? undefined);
                // Use a type guard to ensure activeWorkspaceId is a string for safe comparison
                const isActive = typeof activeWorkspaceId === 'string' && workspace.id === activeWorkspaceId;
